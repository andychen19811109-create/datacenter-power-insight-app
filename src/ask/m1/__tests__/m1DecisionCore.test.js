...
import {
  createM1DecisionResolutionTransport,
} from "../m1DecisionResolutionTransport.js";
...
test("dedicated transport sends exact inputs and enforces published workflow identity", async () => {
  const confirmedInput = createConfirmedInputFixture();
  const request = buildM1DecisionResolutionRequest({ confirmedInput });
  const decisionState = createDecisionStateFixture({ confirmedInput });
  let observed;
  const transport = createM1DecisionResolutionTransport({
    apiUrl: "https://dify.example.test/v1/workflows/run",
    apiKey: "test-only-key",
    expectedWorkflowId: "workflow-version-test",
    fetchImpl: async (url, options) => {
      ...
      return {
        ...
        json: async () => ({
          ...
          data: {
            id: "run-test",
            workflow_id: "workflow-version-test",
            status: "succeeded",
            outputs: { decision_state_json: JSON.stringify(decisionState) },
          },
        }),
      };
    },
  ...
  assert.equal(observed.url, "https://dify.example.test/v1/workflows/run");
  assert.equal(observed.options.method, "POST");
  assert.equal(sent.response_mode, "blocking");
  assert.deepEqual(sent.inputs, request.inputs);
  assert.equal(response.workflowId, "workflow-version-test");
  assert.equal(
    parseM1DecisionResolutionResponse({ confirmedInput, response }).schema_version,
    M1_DECISION_STATE_SCHEMA_VERSION,
  );
});
