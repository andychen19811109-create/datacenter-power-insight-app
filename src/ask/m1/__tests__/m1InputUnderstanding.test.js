...
test("input validation remains independent from clarification and accepts a canonical architecture subject", () => {
  ...
  assert.equal(validateM1InputContext(needsClarification).ok, true);
  ...
  assert.equal(validateM1InputContext(architectureDecision).ok, true);
});

test("input validation rejects a non-canonical decision intent without eligibility routing", () => {
  assert.equal(validateM1InputContext(validContext()).ok, true);
  const invalidIntent = validContext();
  invalidIntent.decision_intent = "NOT_CANONICAL";
  invalidIntent.field_provenance.decision_intent.value = "NOT_CANONICAL";
  const validation = validateM1InputContext(invalidIntent);
  assert.equal(validation.ok, false);
  assert.ok(validation.errors.includes("decision_intent_not_canonical"));
});
