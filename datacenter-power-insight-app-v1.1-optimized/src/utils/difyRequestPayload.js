...
const buildExtraContext = ({ question, filters, insightContext, resolvedContext, guardrail }) => {
  ...
  return [
    ...
    "",
    "【Clean R1 Product Planning Contract】",
    ...
    `- pageLinkage=${cleanR1Contract?.pageLinkageStatement || "Use local Product Planning Card and page synthesis contract."}`,
    ...
    "",
    "【输出要求】",
    "必须按 Product Planning Card 生成五看三定正文，不得只输出摘要或泛化顾问话术。",
    "必须包含投入等级 L0-L4、核心结论、证据边界、验证门槛、进入路径、关键风险、下一步动作和退出条件。",
    "不得编造 TAM、ROI、收入、客户案例、上市时间、认证状态或技术参数。",
    "如果外部输出与本地 Product Planning Card 冲突，必须服从本地合同。",
    `原始问题：${question}`,
  ].join("\n");
};
